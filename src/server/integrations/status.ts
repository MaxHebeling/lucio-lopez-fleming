 ...
 * - con configuración no cambia nada: `callIntegration` la marca `active` tras la primera llamada exitosa.
...
export async function reflectIntegrationConfig(db: Executor, key: string, configured: boolean, reason?: string): Promise<void> {
  ...
  // Configurada no significa que funcione: pasa a `active` recién tras una llamada exitosa (callIntegration).
}
