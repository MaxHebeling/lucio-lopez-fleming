...
export async function dispatchPendingEvents(db: Database, limit = 100): Promise<{ events: number; jobs: number }> {
  return db.transaction().execute(async (trx) => {
    const events = await sql<{ id: string; event_type: string }>`
      select id, event_type from domain_events where dispatched_at is null
...
registerJobHandler("automation.run", async (payload, ctx) => {
  ...
  try {
    ...
    for (const [i, action] of actions.entries()) {
      ...
      if (!handler) throw new PermanentJobError(`Acción desconocida: ${type}`);
      results.push(
        await handler(action, { db: ctx.db, actor: ctx.actor, event, automationKey: auto.key, dedupeBase: `auto:${auto.key}:${ev.id}:${i}` }),
      );
    }
