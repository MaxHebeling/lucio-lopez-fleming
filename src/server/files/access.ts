...
export async function authorizeFileAccess(db: Executor, actor: Actor, fileId: string): Promise<FileAccess> {
  ...
  const refs = await sql<{ entity: string; property_id: string | null; owner_contact_id: string | null; visible_to_owner: boolean | null }>`
    select 'property_document' as entity, d.property_id, null::uuid as owner_contact_id, d.visible_to_owner
      from property_documents d where d.file_id = ${fileId} and d.deleted_at is null
    union all
    select 'property_media', m.property_id, null, null
      from property_media m where (m.file_id = ${fileId} or m.original_file_id = ${fileId}) and m.deleted_at is null
    union all
    select 'owner_report', r.property_id, r.owner_contact_id, null from owner_reports r where r.file_id = ${fileId}
    union all
    select 'social_asset', null, null, null from social_assets s where s.file_id = ${fileId}`.execute(db);
  ...
  for (const ref of refs.rows) {
    if (actor.kind === "staff" || actor.kind === "system") {
      ...
      if (ref.entity === "owner_report" && ref.owner_contact_id === actor.contactId) return { ok: true, file: servable };
