...
/** Logo horizontal: monograma LLF (ladrillo) + wordmark tipográfico + bajada «Buenos negocios». Enlaza al inicio. */
export function Logo({ className, onDark = false, onClick }: { className?: string; onDark?: boolean; onClick?: () => void }) {
  return (
    <Link href="/" onClick={onClick} className={`inline-flex items-center gap-3 ${className ?? ""}`}>
      ...
      {/* Nombre accesible = texto visible + destino (WCAG 2.5.3: quien usa control por voz dice lo que ve). */}
